<template>
    <h1 style="text-align: center;">猜动漫 | 我的题目管理</h1>
    <h2 style="text-align: center;">
        <router-link to="/">首页</router-link> | 
        <span v-if="admin === 'true'"><router-link to="/animeguess/admin">全站题目</router-link> | </span>
        <router-link to="/animeguess/add">添加题目</router-link>
    </h2>
    <h3 style="text-align: center;">
        {{ local.questions.length }} 个题目（全站共 {{ total }} 个）
    </h3>
    <hr />
            ...
            <table class="table table-striped" style="margin: 0px auto;">
                ...
                <tbody>
                    <tr v-for="(question, index) in local.questions" :key="question.id">
                        <td><span class="large">{{ question._id.slice(question._id.length - 6) }}</span><span class="small">{{ index + 1 }}</span></td>
                        <td><router-link style="margin-right: 5px" :to="'/animeguess/' + question._id">{{ question.title }}</router-link></td>
                        <td>{{ question.from }}</td>
...
if (!token) {
    ...
    (async function () {
        ...
        while (local.questions.length < thistotal) {
            const res = await fetch(config.apiurl + '/animeguess?page=' + page, { headers: { 'Authorization': 'Bearer ' + token } })
